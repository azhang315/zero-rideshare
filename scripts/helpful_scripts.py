...
INITIAL_PRICE_FEED_VALUE = 2000000000000000000000 # 2000
DECIMALS = 18
...
def get_contract(contract_name):
    ...
    if network.show_active() in NON_FORKED_LOCAL_BLOCKCHAIN_ENVIRONMENTS: # LOCAL ENVIRONMENT (NON-FORK)
        if len(contract_type) <= 0: # No prexisting mocks deployed
            deploy_mocks()
        contract = contract_type[-1] # Access most recent mock
    else:
        try: # LIVE NET OR FORKED LOCAL ENVIRONMENT
